# Start Both Servers for HOMA Clinic MVP
...
# Get full absolute project path
$projectPath = if ($PSScriptRoot) { $PSScriptRoot } else { (Get-Location).Path }
$serverPath = (Resolve-Path (Join-Path $projectPath "server")).Path
$webPath = (Resolve-Path (Join-Path $projectPath "web")).Path
